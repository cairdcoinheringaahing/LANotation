...
|:----------|:-------------|:-----------------------------------|
| Treant | `Treant` | |
| Taunting Satyr | `TSatyr` | `TSatyr taunt <hex>` |
| Wisp | `Wisp` | `Wisp heal <hex>` / `Wisp: Break <hex>` |
| Deepwood Protector | `DProc` | `DProc Man <hex>` |
| Glade Hawk | `GHawk` | |
| Tethir Archer | `TArcher` | `TArcher <hex 1> shoot <hex 2>; TArcher <hex 3>` |
| Deepwood Sentinel | `DSen` | `DSen Man <hex>` |
| Norfang Hunter | `NHunter` | |
| Tethir Fletcher | `TFletch` | `TFletch: Pier1 <hex>` / `TFletch: Net <hex>` / `TFletch: Hook <hex>` |
| Deepwood Mystic | `DMystic` | `DMystic heal <hex 1> <hex 2>` / `DMystic clear <debuff> <hex>` |
| Norfang Sorcerer | `NSorc` | `NSorc: Silence <hex>` / `NSorc: Break <hex>` / `NSorc: Root <hex>` |
| Tethir Grand Archer | `TGA` | `TGA <hex 1> shoot <hex 2>; TGA <hex 3>` |
| Witchhunter | `WHunter` | |
| Arbormage | `Arbor` | `Arbor: +XPDR <hex>` / `Arbor: Stead <hex>; Arbor: SProof <hex>` |
| Centaur Guard | `CGuard` | `CGuard Man <hex>` |
| Creeping Palisade | `Palisade` | |
...
| Norfang High Sorcerer | `NHS` | `NHS: Silence <hex>` / `NHS: Break <hex>` / `NHS: Root <hex>` / `NHS: Break <hex>; NHS: Root <hex>` |
